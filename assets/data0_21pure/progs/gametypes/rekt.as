...
String @GT_ScoreboardMessage( uint maxlen )
{
	...
	for ( int j = 0; @alpha.ent( j ) != null; j++ )
	{
        ...
        // "AVATAR Name Clan Score Ping"
		entry = "&p " + playerID + " " + playerID + " " + client.clanName + " " + client.stats.score + " " + client.ping + " ";
		if ( scoreboardMessage.len() + entry.len() < maxlen )
			scoreboardMessage += entry;
	}
...
void GT_InitGametype()
{
	...
	G_ConfigString( CS_SCB_PLAYERTAB_LAYOUT, "%a l1 %n 112 %s 52 %i 52 %l 48" );
	G_ConfigString( CS_SCB_PLAYERTAB_TITLES, "AVATAR Name Clan Score Ping" );
